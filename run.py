...
if __name__ == "__main__":
    ...
    node_weight = np.array([], dtype=np.int32)
    edge_weight = np.array([], dtype=np.int32)
    
    if args.node_weight_path != "":
        node_weight = np.load(args.node_weight_path).astype(np.int32)
    elif args.node_weight_path != "":
        edge_weight = np.load(args.edge_weight_path).astype(np.int32)
    ...
    print(f"fininished metis in {round(end - start)} secs and saving to {args.output_path}")
