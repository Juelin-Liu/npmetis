...
namespace pmetis
{
    std::vector<VertexPIDType> mpi_metis_assignment(MPI_Comm *comm,
                                                    ...
                                                    std::span<WeightType> edge_weight)
    {
        ...
        idx_t wgtflag = 0;
        if (edge_weight.size() && node_weight.size()) wgtflag = 3;
        else if (edge_weight.size()) wgtflag = 1;
        else if (node_weight.size()) wgtflag = 2;
        else wgtflag = 0; 
